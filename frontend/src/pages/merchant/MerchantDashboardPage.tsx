...
export function MerchantDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-xl font-semibold text-zinc-100">Merchant Dashboard</h1>

      <div className="grid grid-cols-4 gap-4">
        {statsQuery.isLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-24 bg-zinc-800 border border-zinc-700 rounded-md animate-pulse" />
          ))
        ) : statsQuery.isError ? (
          <p className="col-span-4 text-sm text-red-400">Failed to load stats.</p>
        ) : (
          <>
